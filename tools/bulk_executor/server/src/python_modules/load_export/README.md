...
The load-export command supports an optional `--transform` parameter that specifies a Python module containing transform functions. These functions receive deserialized export records and can filter, mutate, or fan-out items before they are written to DynamoDB.

The transform module must contain one or both of these functions depending on the export type being loaded:
...
**Return behavior:**
- `return [record]` - load as-is. If `new_image` is present it becomes a PUT, if `new_image` is `None` it becomes a DELETE
...
**Example - add an attribute during load:**
```python
...
- **`pii_remove_attribute.py`** - Strip PII attributes (e.g. `Name`) from items during load, skipping any that are the partition key or sort key.
