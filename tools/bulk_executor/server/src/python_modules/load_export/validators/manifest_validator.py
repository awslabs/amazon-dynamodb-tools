...
class ManifestValidator:
    ...
    def validate_and_parse_manifests(self, path_resolver: ExportPathResolver) -> Dict:
        ...
        """
        log.debug(f"Starting manifest validation for export path: {path_resolver}")
        ...
        log.debug(f"data_base_path {data_base_path}")
        log.debug(f"manifest_base_path {manifest_base_path}")
        
        # Step 1: Validate manifest-summary.json MD5 checksum
        log.debug("Manifest validator-1: Validating manifest-summary.json MD5 checksum...")
        ...
        log.debug("Manifest validator-2: Parsing manifest-summary.json...")
        try:
        ...
        log.debug(f"Parsed manifest-summary.json: itemCount={total_item_count:,}, outputFormat={output_format}, exportType={export_type}, exportTime={export_time}, exportFromTime={export_from_time}, exportToTime={export_to_time}")
        
        # Step 3: Extract table name from ARN
        log.debug("Manifest validator-3: Extracting table name from arn...")
        ...
        log.debug("Manifest validator-4: Validating output format...")
        ...
        log.debug("Manifest validator-5: Validating export views...")
        output_view = None
        if export_type == 'INCREMENTAL_EXPORT':
            ...
            log.debug("Output view validated successfully")
        
        # Step 6: Validate manifest-files.json MD5 checksum
        log.debug("Manifest validator-6: Validating manifest-files.json MD5 checksum..." + manifest_files_key)
        ...
        log.debug("Manifest validator-7: Parsing manifest-files.json...")
        data_files = []
        try:
        ...
        log.debug(f"Parsed {len(data_files):,} data file entries from manifest-files.json")

        # Step 8: Calculate and validate item count consistency
        log.debug("Manifest validator-8: Validating item count consistency...")
