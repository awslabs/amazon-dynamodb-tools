...
def get_export_file_paths(
    ...
    for data_file in data_files:
        ...
        #log.debug(f"Will process file: {file_key} (expected items: {expected_count:,})")
    
    if skipped_empty:
        log.debug(f"Skipped {skipped_empty} empty data file(s)")

    log.debug(f"Total files to process: {len(file_paths):,}")
    log.debug(f"Total expected items: {total_expected_items:,}")
