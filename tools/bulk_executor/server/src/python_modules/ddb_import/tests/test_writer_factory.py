...
class TestWriterFactory:
    ...
    def test_returns_batch_writer_for_full_only_import(self):
        """Test that FULL_ONLY import returns BatchWriter."""
        writer = WriterFactory.create_writer(ImportType.FULL_ONLY)
        assert isinstance(writer, BatchWriter)
    
    def test_returns_item_writer_for_incremental_import(self):
        """Test that FULL_INCREMENTAL import returns ItemWriter."""
        writer = WriterFactory.create_writer(ImportType.FULL_INCREMENTAL)
    ...
    def test_returns_item_writer_for_incremental_only_import(self):
        ...
        writer = WriterFactory.create_writer(ImportType.INCREMENTAL_ONLY)
        assert isinstance(writer, ItemWriter)
    
    def test_factory_creates_different_writer_instances(self):
        """Test that factory creates new instances each time."""
        writer1 = WriterFactory.create_writer(ImportType.FULL_ONLY)
        writer2 = WriterFactory.create_writer(ImportType.FULL_ONLY)
