...
# Attribute names to remove if they are not the partition key or sort key.
...
def _remove_pii(item, key_names):
    """Remove PII attributes from the item, skipping any that are key attributes."""
...
def transform_full_record(record: FullExportRecord) -> list[FullExportRecord]:
    """
    Example: Remove PII attributes from items during import.

    Removes attributes listed in PII_ATTRIBUTES from each item, unless the
    attribute is the partition key or sort key (in which case it is left
    intact — removing a key attribute would break the import).
    ...
    Args:
        record.item: Deserialized Python dict.
            Example: {"Id": 42, "Name": "Alice", "status": "active"}
            NOT: {"Id": {"N": "42"}, "Name": {"S": "Alice"}}
...
def transform_incremental_record(record: IncrementalExportRecord) -> list[IncrementalExportRecord]:
    ...
    Behavior:
        - If new_image exists (PUT/update): remove PII attributes from new_image
    ...
    if record.new_image:
        key_names = _get_key_attribute_names(record.table_key_schema)
        _remove_pii(record.new_image, key_names)
    return [record]
