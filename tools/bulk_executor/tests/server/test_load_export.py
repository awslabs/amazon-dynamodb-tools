"""Unit tests for the load_export orchestrator (__init__.py)."""

import pytest
from unittest.mock import Mock, MagicMock, patch, call

from python_modules.load_export import run, _validate, _estimate_cost, _read_and_parse, _apply_transform_stage, _write, _report
from python_modules.load_export.utils.enums import ExportLoadType, Operation
...
class TestValidate:
    @patch('python_modules.load_export.get_and_print_dynamodb_table_info')
    @patch('python_modules.load_export.boto3')
    def test_happy_path_returns_validation_dict(self, mock_boto3, mock_table_info, key_schema, table_info, manifest_data_full):
        ...
        with patch('python_modules.load_export.S3Validator') as MockS3Val, \
             patch('python_modules.load_export.ManifestValidator') as MockManVal, \
             patch('python_modules.load_export.DataFileValidator') as MockDFVal, \
             patch('python_modules.load_export.KeySchemaValidator') as MockKSVal:
            ...
            result = _validate(path_resolver, 'my-table', step=1)
    ...
    def test_zero_item_count_returns_none(self, mock_boto3, mock_table_info, table_info):
        ...
        with patch('python_modules.load_export.S3Validator'), \
             patch('python_modules.load_export.ManifestValidator') as MockManVal, \
             patch('python_modules.load_export.DataFileValidator'), \
             patch('python_modules.load_export.KeySchemaValidator'):
    ...
    def test_manifest_validation_raises_propagates(self, mock_boto3, mock_table_info, table_info):
        ...
        with patch('python_modules.load_export.S3Validator'), \
            ...
            with pytest.raises(ValueError, match="Invalid manifest"):
                _validate(path_resolver, 'my-table', step=1)
    ...
    def test_s3_path_validation_raises_propagates(self, mock_boto3, mock_table_info, table_info):
        ...
        with patch('python_modules.load_export.S3Validator') as MockS3Val, \
             patch('python_modules.load_export.ManifestValidator'), \
             patch('python_modules.load_export.DataFileValidator'), \
             patch('python_modules.load_export.KeySchemaValidator'):
...
class TestEstimateCost:
    @patch('python_modules.load_export.get_and_print_table_write_cost')
    def test_calls_cost_estimator_and_increments_step(self, mock_cost):
        table_info = {'table_name': 'tbl'}
        manifest_data = {'total_item_count': 1000}
        key_schema_result = {'avg_item_size': 500}

        step = _estimate_cost(table_info, manifest_data, key_schema_result, step=7)
    ...
    def test_zero_avg_item_size(self, mock_cost):
        ...
        step = _estimate_cost(table_info, manifest_data, key_schema_result, step=5)
...
class TestReadAndParse:
    @patch('python_modules.load_export.ParserFactory')
    @patch('python_modules.load_export.get_export_file_paths')
    def test_full_export_uses_full_parser(self, mock_get_paths, mock_parser_factory, mock_spark_context, manifest_data_full, key_schema):
        ...
        records_rdd, export_load_type, parser, total_expected, step = _read_and_parse(
            mock_spark_context, manifest_data_full, path_resolver, key_schema, step=8
        )
    ...
    def test_incremental_export_uses_incremental_parser(self, mock_get_paths, mock_parser_factory, mock_spark_context, manifest_data_incremental, key_schema):
        ...
        records_rdd, export_load_type, parser, total_expected, step = _read_and_parse(
            mock_spark_context, manifest_data_incremental, path_resolver, key_schema, step=8
        )
...
class TestApplyTransformStage:
    ...
    @patch('python_modules.load_export.load_transform_module')
    def test_full_export_with_transform_uses_transform_full_record(self, mock_load_transform, mock_spark_context, key_schema):
        ...
        items_rdd, transform_active, excl_acc, mod_acc = _apply_transform_stage(
            mock_spark_context, records_rdd, ExportLoadType.FULL, mock_parser,
            'my_transform', key_schema, error_acc
        )

        assert transform_active is True
        assert excl_acc is acc_mock
        assert mod_acc is acc_mock
        mock_load_transform.assert_called_once_with('my_transform')
...
class TestWrite:
    @patch('python_modules.load_export.WriterFactory')
    def test_happy_path_writes_and_returns_accumulator(self, mock_writer_factory, mock_spark_context):
        ...
        result_acc, step = _write(
            mock_spark_context, items_rdd, ExportLoadType.FULL, 'my-table',
            rate_config, monitor_options, error_acc, debug_acc, step=10
        )
    ...
    def test_error_accumulator_nonempty_raises(self, mock_writer_factory, mock_spark_context):
        ...
        with pytest.raises(Exception, match="Write failed: throttled"):
            _write(
                mock_spark_context, items_rdd, ExportLoadType.FULL, 'my-table',
                Mock(), {}, error_acc, debug_acc, step=10
            )
...
class TestRunOrchestrator:
    @patch('python_modules.load_export.RateLimiterAggregator')
    @patch('python_modules.load_export.RateLimiterSharedConfig')
    @patch('python_modules.load_export.get_dynamodb_throughput_configs')
    @patch('python_modules.load_export._report')
    @patch('python_modules.load_export._write')
    @patch('python_modules.load_export._apply_transform_stage')
    @patch('python_modules.load_export._read_and_parse')
    @patch('python_modules.load_export._estimate_cost')
    @patch('python_modules.load_export._validate')
    def test_happy_path_full_pipeline(self, mock_validate, mock_estimate, mock_read, mock_transform, mock_write, mock_report,
                                      ...
                                      mock_job, mock_spark_context, mock_glue_context, parsed_args_full, key_schema):
        ...
        mock_estimate.return_value = 8
        mock_read.return_value = (Mock(), ExportLoadType.FULL, Mock(), 100, 10)
        mock_transform.return_value = (Mock(), False, None, None)
        mock_write.return_value = (Mock(value=100), 12)

        run(mock_job, mock_spark_context, mock_glue_context, parsed_args_full)
    ...
    def test_validation_returns_none_exits_early(self, mock_validate, mock_throughput, mock_rl_config, mock_rl_agg,
                                                  mock_job, mock_spark_context, mock_glue_context, parsed_args_full):
        mock_spark_context.accumulator = Mock(return_value=Mock(value=[]))
    ...
    def test_debug_accumulator_enabled(self, mock_validate, mock_estimate, mock_read, mock_transform, mock_write, mock_report,
                                        mock_throughput, mock_rl_config, mock_rl_agg,
                                        mock_job, mock_spark_context, mock_glue_context, key_schema):
        parsed_args = {
            'table': 'my-table',
            's3_path': 's3://my-bucket/prefix/AWSDynamoDB/export-001',
            'transform': None,
    ...
    @patch('python_modules.load_export._estimate_cost')
    @patch('python_modules.load_export._validate')
    def test_run_with_transform_passes_transform_name(self, mock_validate, mock_estimate, mock_read, mock_transform, mock_write, mock_report,
                                                       mock_throughput, mock_rl_config, mock_rl_agg,
                                                       mock_job, mock_spark_context, mock_glue_context, parsed_args_with_transform, key_schema):
        mock_spark_context.accumulator = Mock(return_value=Mock(value=[]))
        ...
        run(mock_job, mock_spark_context, mock_glue_context, parsed_args_with_transform)
