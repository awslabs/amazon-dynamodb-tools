...
def checkpoints(ctx: click.Context, cleanup: bool, max_age: int) -> None:
    ...
    try:
        ...
        # Show usage hints
        click.echo("  Commands:")
        click.echo("   dynamodb-optima resume --latest          # Resume latest checkpoint")
        click.echo("   dynamodb-optima resume <operation_id>    # Resume specific checkpoint")
        click.echo("   dynamodb-optima checkpoints --cleanup    # Clean up old checkpoints")
