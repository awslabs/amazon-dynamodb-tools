...
A comprehensive CLI and GUI tool that analyzes DynamoDB usage patterns across AWS Organizations to identify cost optimization opportunities. Consolidates the functionality of `capacity-mode-evaluator`, `table_class_optimizer`, and `dmetrics` into a single, enterprise-ready solution.
...
Metrics Collector v2 provides three types of cost optimization analysis for DynamoDB:
...
metrics-collector collect --start-date 2026-01-01 --end-date 2026-01-14
```
...
metrics-collector collect-pricing
```
...
# Export to CSV
metrics-collector list-recommendations --format csv --output recs.csv
```
...
# AWS Organizations
metrics-collector discover --use-org [--role-name MetricsCollectorRole]

# Resume from checkpoint
metrics-collector discover --resume
```
...
metrics-collector collect --tables table1,table2 --days 7

# Resume interrupted collection
metrics-collector collect --resume
```
...
metrics-collector collect-pricing --force-refresh
```
...
metrics-collector collect-cur --format parquet
```
...
# Filter by type
metrics-collector list-recommendations --type capacity

# Filter by savings
metrics-collector list-recommendations --min-savings 100

# Export to CSV
metrics-collector list-recommendations --format csv --output recs.csv

# Show only actionable recommendations
metrics-collector list-recommendations --status pending
```
...
metrics-collector status --verbose
```
...
metrics-collector health --verbose
```
...
# Custom port
metrics-collector gui --port 8080

# Custom theme
metrics-collector gui --theme dark
```
...
# Database
DATABASE_URL=data/metrics_collector.db
...
Edit `src/metrics_collector/config.py` for programmatic configuration or create a custom config file:

```python
from metrics_collector.config import Settings
...
metrics-collector analyze-capacity --min-savings 1
...
metrics-collector collect --regions us-east-1 --days 7

# Reduce batch size
metrics-collector collect --days 14 --batch-size 500
...
metrics-collector collect-cur --bucket my-cur-bucket --prefix reports/
```
...
# Or use --verbose flag
metrics-collector collect --days 7 --verbose
```

### Log Locations

- Application logs: `logs/metrics_collector.log`
- Database: `data/metrics_collector.db`
...
3. Open a new issue with:
   ...
   - Error message
   - Log excerpt
   - `metrics-collector version` output
...
for profile in prod-account-1 prod-account-2; do
  AWS_PROFILE=$profile metrics-collector discover
  AWS_PROFILE=$profile metrics-collector collect --days 14
done
```
...
0 2 * * * cd /path/to/metrics-collector-v2 && metrics-collector collect --days 1

# Weekly analysis
0 3 * * 0 cd /path/to/metrics-collector-v2 && metrics-collector analyze-capacity --days 7

# Monthly table class analysis
0 4 1 * * cd /path/to/metrics-collector-v2 && metrics-collector analyze-table-class --months 1
```
...
```python
from metrics_collector.database import get_connection
...
metrics-collector list-recommendations --format json > recs.json
...
- **dmetrics** - Original metrics collection framework (reference)
