...
from metrics_collector.gui.database import get_capacity_recommendations
...
def render_capacity_analysis(connection, filters: RecommendationFilter):
    ...
    try:
        # Get recommendations
        recommendations = get_capacity_recommendations(connection, filters)

        if not recommendations:
            st.info(
                "No capacity mode recommendations found matching the current filters.\n\n"
                "Run `metrics-collector analyze-capacity` to generate recommendations."
            )
            return
