...
logger = get_logger("dmetrics.aws.collector")
    ...
    async def collect_metrics(
        ...
        try:
            ...
            if total_discovered == 0:
                ...
                logger.info(
                    "Try running 'dmetrics discover' first to populate the database"
                )
    ...
    async def resume_collection(
        ...
        if state.status == "COMPLETED":
            raise CollectionError(
                ...
                f"Use 'metrics-collector collect' to start a new collection."
            )
