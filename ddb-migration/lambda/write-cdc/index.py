...
def lambda_handler(event, context):
    ...
    try:
        ...
        return {
            'statusCode': 200,
            'body': json.dumps(f'Successfully processed {processed_count} messages, Failed: {len(failed_messages)}')
        }
...
def process_results(unprocessed_items, all_items, failed_messages):
    for item in unprocessed_items:
        failed_messages.append({
            ...
            'ReceiptHandle': next((record['receiptHandle'] for record in event['Records']
                                   if record['messageId'] == item['sqs_message_id']), None)
        })
...
def send_to_dlq_with_retry(messages):
    for message in messages:
        ...
        if retry_count == MAX_RETRIES:
            print(f"CRITICAL: Failed to send message {message['Id']} to DLQ after {MAX_RETRIES} attempts")
