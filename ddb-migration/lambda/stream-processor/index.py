            ...
            try:
                ...
                if "Failed" in response and response["Failed"]:
                    failed_messages = [
                        msg
                        for msg in batch
                        if msg["Id"] in [fail["Id"] for fail in response["Failed"]]
                    ]
                    batch = failed_messages
                    retry_count += 1
                    print(
                        f"Retry {retry_count} for {len(failed_messages)} failed messages"
                    )
                else:
                    print(
                        f"Successfully sent {len(response['Successful'])} messages to FIFO queue"
                    )
                    break
...
def send_to_dlq_with_retry(messages):
    for message in messages:
        ...
        if retry_count == MAX_DLQ_RETRIES:
            print(
                f"CRITICAL: Failed to send message {message['Id']} to DLQ after {MAX_DLQ_RETRIES} attempts"
            )
